// Gate 8K · Playwright · UI PIN flow complet + Coach data-testid count.
...
(async () => {
  ...
  const PORT = process.env.YEMA_CHILD_UI_PORT || "3350";

  function fail(step: string, msg: string, code = 1): never {
    console.error(`[child-ui] STEP ${step} FAIL · ${msg}`);
    process.exit(code);
  }
  ...
  async function ensureCoachTemp(email: string): Promise<{ id: string; supabaseId: string }> {
    ...
    if (!authUser) {
      const { data, error } = await admin.auth.admin.createUser({
        ...
        user_metadata: { fixture: "GATE_8K_TEMP", roles: ["STUDENT"], active_space: "STUDENT" },
      });
    ...
    const dbUser = await db.user.upsert({
      where: { email },
      update: { supabaseId, role: "STUDENT" as never, fullName: `GATE 8K ${email.split("@")[0]}`, onboardingDone: true },
      create: { email, supabaseId, role: "STUDENT" as never, fullName: `GATE 8K ${email.split("@")[0]}`, onboardingDone: true },
      select: { id: true },
    });
  ...
  async function main() {
    console.log("[child-ui] STEP 1 · fixtures QA");
    spawnSync("node", ["scripts/test-baseline/yema-qa-fixtures.mjs"], { stdio: "inherit", env: process.env });

    const ts = Date.now();

    console.log("[child-ui] STEP 2 · fixtures Coach A/B + enfants distincts");
    const coachAEmail = `temp_gate8k_coach_a_${ts}@example.com`;
    const coachBEmail = `temp_gate8k_coach_b_${ts}@example.com`;
    const coachA = await ensureCoachTemp(coachAEmail);
    const coachB = await ensureCoachTemp(coachBEmail);

    const hhAId = `test_yema_qa_gate8k_hh_a_${ts}`;
    const hhBId = `test_yema_qa_gate8k_hh_b_${ts}`;
    ...
    const childAId = `test_yema_qa_gate8k_child_a_${ts}`;
    const childBId = `test_yema_qa_gate8k_child_b_${ts}`;
    ...
    const cAId = `test_yema_qa_gate8k_circle_a_${ts}`;
    const cBId = `test_yema_qa_gate8k_circle_b_${ts}`;
    ...
    console.log(`[child-ui] STEP 3 · next start port ${PORT}`);
    ...
    console.log("[child-ui] STEP 4 · Playwright chromium · UI PIN flow + Coach count + network");
    const pw = spawnSync("npx", [
      "playwright", "test", "--config", "playwright.final-browser-acceptance.config.ts",
      "tests/e2e/final-browser-acceptance/gate8k-child-ui-flow.spec.ts",
    ], {
      stdio: "inherit",
      env: {
        ...process.env,
        PLAYWRIGHT_BASE_URL: `http://127.0.0.1:${PORT}`,
        GATE8K_COACH_A_EMAIL: coachAEmail,
        GATE8K_COACH_B_EMAIL: coachBEmail,
        YEMA_CHILD_SESSION_SECRET: hmacSecret,
      },
    });
    if (pw.status !== 0) fail("4", `Playwright exit ${pw.status}`);
    console.log(`  ✓ Playwright all green`);
  }

  async function runCleanup() {
    console.log("[child-ui] CLEANUP · restauration finally");
    while (cleanup.length) {
      try { await cleanup.pop()!(); } catch (e) { console.error(`  · cleanup fail · ${(e as Error).message}`); }
    }
    const leakUsers = await db.user.count({ where: { email: { startsWith: "temp_gate8k_" } } });
    const leakChildren = await db.childProfile.count({ where: { id: { startsWith: "test_yema_qa_gate8k_" } } });
    const leakCircles = await db.circle.count({ where: { id: { startsWith: "test_yema_qa_gate8k_" } } });
    const leakHouseholds = await db.household.count({ where: { id: { startsWith: "test_yema_qa_gate8k_" } } });
  ...
  try { await main(); }
  catch (e) { console.error(`[child-ui] ERROR · ${(e as Error).message}`); process.exitCode = 1; }
