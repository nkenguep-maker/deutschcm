#!/usr/bin/env node
// P0.8 · Runtime gate for classes/centres/provider/application tables.
//
...
function fail(message) {
  throw new Error(`P0.8 RLS gate failed: ${message}`);
}
...
try {
  ...
  if (policies.rows.length !== 0) {
    fail(
      `P0.8 tables must remain deny-by-default; policies found on: ${[
  ...
  process.stdout.write(
    `P0.8 RLS GATE OK · ${TABLES.length}/${TABLES.length} classes/centres tables RLS-enabled · client grants=0 · policies=0 · trusted server read=OK\n`,
  );
} finally {
  await db.end().catch(() => {});
}
