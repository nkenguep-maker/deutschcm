#!/usr/bin/env node
// P0.5 · Runtime gate for the pedagogical catalogue.
//
...
function fail(message) {
  throw new Error(`P0.5 RLS gate failed: ${message}`);
}
...
try {
  ...
  if (policies.rows.length !== 0) {
    fail(
      `catalogue tables must remain deny-by-default; policies found on: ${[
  ...
  process.stdout.write(
    `P0.5 RLS GATE OK · ${TABLES.length}/${TABLES.length} pedagogical catalogue tables RLS-enabled · client grants=0 · policies=0 · trusted server read=OK\n`,
  );
} finally {
  await db.end().catch(() => {});
}
