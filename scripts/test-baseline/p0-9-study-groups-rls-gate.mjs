#!/usr/bin/env node
// P0.9 · Runtime gate for study-group/social membership tables.
//
...
function fail(message) {
  throw new Error(`P0.9 RLS gate failed: ${message}`);
}
...
try {
  ...
  if (policies.rows.length !== 0) {
    fail(
      `study-group tables must remain deny-by-default; policies found on: ${[
  ...
  process.stdout.write(
    `P0.9 RLS GATE OK · ${TABLES.length}/${TABLES.length} study-group tables RLS-enabled · client grants=0 · policies=0 · trusted server read=OK\n`,
  );
} finally {
  await db.end().catch(() => {});
}
