#!/usr/bin/env node
// P0.3 · Runtime gate for the family ownership boundary.
//
// Read-only verification. Run only through the strict P-1 wrapper.
// child_profiles is included because it is the canonical child runtime table
// and must remain deny-by-default together with the ownership tables.
...
function fail(message) {
  throw new Error(`P0.3 RLS gate failed: ${message}`);
}
...
try {
  ...
  if (policies.rows.length !== 0) {
    fail(
      `family boundary must remain deny-by-default; policies found on: ${[
  ...
  process.stdout.write(
    `P0.3 RLS GATE OK · ${TABLES.length}/${TABLES.length} family tables RLS-enabled · client grants=0 · policies=0 · trusted server read=OK\n`,
  );
} finally {
  await db.end().catch(() => {});
}
