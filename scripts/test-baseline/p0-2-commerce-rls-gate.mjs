#!/usr/bin/env node
// P0.2 · Runtime gate for commerce + entitlement RLS.
//
...
function fail(message) {
  throw new Error(`P0.2 RLS gate failed: ${message}`);
}
...
try {
  ...
  if (policies.rows.length !== 0) {
    fail(
      `commerce tables must remain deny-by-default; policies found on: ${[
  ...
  // Prove the trusted server connection still reads every protected table.
  for (const table of TABLES) {
    await db.query(`select 1 from public.${table} limit 1`);
  }

  process.stdout.write(
    `P0.2 RLS GATE OK · ${TABLES.length}/${TABLES.length} commerce tables RLS-enabled · client grants=0 · policies=0 · trusted server read=OK\n`,
  );
} finally {
  await db.end().catch(() => {});
}
