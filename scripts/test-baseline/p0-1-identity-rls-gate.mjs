#!/usr/bin/env node
// P0.1 · Runtime gate for the first deny-by-default RLS slice.
//
// Read-only verification. It must run only through the P-1 environment guard.
...
function fail(message) {
  throw new Error(`P0.1 RLS gate failed: ${message}`);
}
...
try {
  ...
  if (policies.rows.length !== 0) {
    fail(
      `identity tables must remain deny-by-default; found policies on: ${[
  ...
  // Prove the trusted server connection can still read each table after RLS.
  for (const table of TABLES) {
    await db.query(`select 1 from public.${table} limit 1`);
  }

  process.stdout.write(
    `P0.1 RLS GATE OK · ${TABLES.length}/${TABLES.length} identity tables RLS-enabled · client grants=0 · policies=0 · trusted server read=OK\n`,
  );
} finally {
  await db.end().catch(() => {});
}
