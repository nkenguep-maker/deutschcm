#!/usr/bin/env node
// P0.4 · Runtime gate for user learning/runtime data.
//
...
function fail(message) {
  throw new Error(`P0.4 RLS gate failed: ${message}`);
}
...
try {
  ...
  if (policies.rows.length !== 0) {
    fail(
      `learning-runtime tables must remain deny-by-default; policies found on: ${[
  ...
  process.stdout.write(
    `P0.4 RLS GATE OK · ${TABLES.length}/${TABLES.length} learning-runtime tables RLS-enabled · client grants=0 · policies=0 · trusted server read=OK\n`,
  );
} finally {
  await db.end().catch(() => {});
}
