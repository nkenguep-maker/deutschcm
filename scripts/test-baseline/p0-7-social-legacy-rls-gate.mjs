#!/usr/bin/env node
// P0.7 · Runtime gate for social/notification/legacy messaging tables.
//
...
function fail(message) {
  throw new Error(`P0.7 RLS gate failed: ${message}`);
}
...
try {
  ...
  if (policies.rows.length !== 0) {
    fail(
      `P0.7 tables must remain deny-by-default; policies found on: ${[
  ...
  process.stdout.write(
    `P0.7 RLS GATE OK · ${TABLES.length}/${TABLES.length} social/legacy tables RLS-enabled · client grants=0 · policies=0 · trusted server read=OK\n`,
  );
} finally {
  await db.end().catch(() => {});
}
