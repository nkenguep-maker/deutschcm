    ...
    <main style={{ minHeight: "100vh", background: "var(--espresso)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      ...
      <div style={{ width: "100%", maxWidth: 620 }} className="fadeUp">
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          ...
          <h1 style={{ margin: 0, color: "var(--creme)", fontFamily: "var(--font-fraunces), Georgia, sans-serif", fontWeight: 800, fontSize: 22 }}>Créer votre Centre</h1>
          <p style={{ margin: "6px 0 0", color: "rgba(244, 235, 220, 0.62)", fontSize: 13 }}>Enregistrez votre centre de langues sur la plateforme Yema</p>
        </div>
        ...
        <div style={{ background: "rgba(36, 24, 18, 0.85)", border: "1px solid rgba(244, 235, 220, 0.09)", borderRadius: 18, padding: 28 }}>
          ...
          {step === 0 && (
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              <h2 style={{ margin: 0, color: "var(--creme)", fontFamily: "var(--font-fraunces), Georgia, sans-serif", fontSize: 17 }}>Identité du centre</h2>
              <div>
                <label style={{ color: "rgba(244, 235, 220, 0.62)", fontSize: 11, fontWeight: 600, display: "block", marginBottom: 5 }}>NOM DU CENTRE *</label>
                <input value={form.name} onChange={e => set("name", e.target.value)} placeholder="ex: Centre Deutsch Pro CM" style={inp()} />
                {errors.name && <div style={{ color: "#E5B4B7", fontSize: 11, marginTop: 4 }}>{errors.name}</div>}
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <div>
                  <label style={{ color: "rgba(244, 235, 220, 0.62)", fontSize: 11, fontWeight: 600, display: "block", marginBottom: 5 }}>TYPE DE CENTRE *</label>
                  <select aria-label="Type de centre" value={form.centerType} onChange={e => set("centerType", e.target.value)} style={{ ...inp(), appearance: "none" } as React.CSSProperties}>
                    <option value="">Choisir...</option>
                    {CENTER_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
                  </select>
                  {errors.centerType && <div style={{ color: "#E5B4B7", fontSize: 11, marginTop: 4 }}>{errors.centerType}</div>}
                </div>
                <div>
                  <label style={{ color: "rgba(244, 235, 220, 0.62)", fontSize: 11, fontWeight: 600, display: "block", marginBottom: 5 }}>DATE DE CRÉATION</label>
                  <input type="date" aria-label="foundedAt" value={form.foundedAt} onChange={e => set("foundedAt", e.target.value)} style={{ ...inp(), colorScheme: "dark" }} />
                </div>
              </div>
              <div>
                <label style={{ color: "rgba(244, 235, 220, 0.62)", fontSize: 11, fontWeight: 600, display: "block", marginBottom: 5 }}>N° RCCM <span style={{ fontWeight: 400 }}>(optionnel)</span></label>
                <input value={form.rccm} onChange={e => set("rccm", e.target.value)} placeholder="ex: YAO/RC/0099/B" style={inp()} />
              </div>
              <div>
                <label style={{ color: "rgba(244, 235, 220, 0.62)", fontSize: 11, fontWeight: 600, display: "block", marginBottom: 5 }}>LOGO URL <span style={{ fontWeight: 400 }}>(optionnel)</span></label>
          ...
          {step === 1 && (
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <h2 style={{ margin: 0, color: "var(--creme)", fontFamily: "var(--font-fraunces), Georgia, sans-serif", fontSize: 17 }}>Localisation & contacts</h2>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <div>
                  <label style={{ color: "rgba(244, 235, 220, 0.62)", fontSize: 11, fontWeight: 600, display: "block", marginBottom: 5 }}>RÉGION *</label>
                  <select aria-label="Région" value={form.region} onChange={e => { set("region", e.target.value); set("city", ""); }} style={{ ...inp(), appearance: "none" } as React.CSSProperties}>
                    <option value="">Choisir...</option>
                    {REGIONS.map(r => <option key={r} value={r}>{r}</option>)}
                  </select>
                  {errors.region && <div style={{ color: "#E5B4B7", fontSize: 11, marginTop: 4 }}>{errors.region}</div>}
                </div>
                <div>
                  <label style={{ color: "rgba(244, 235, 220, 0.62)", fontSize: 11, fontWeight: 600, display: "block", marginBottom: 5 }}>VILLE *</label>
                  <select aria-label="Ville" value={form.city} onChange={e => set("city", e.target.value)} disabled={!form.region} style={{ ...inp(), appearance: "none", opacity: form.region ? 1 : 0.5 } as React.CSSProperties}>
                    <option value="">Choisir...</option>
                    {(CITIES[form.region] ?? []).map(c => <option key={c} value={c}>{c}</option>)}
                  </select>
                  {errors.city && <div style={{ color: "#E5B4B7", fontSize: 11, marginTop: 4 }}>{errors.city}</div>}
                </div>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <div>
                  <label style={{ color: "rgba(244, 235, 220, 0.62)", fontSize: 11, fontWeight: 600, display: "block", marginBottom: 5 }}>QUARTIER</label>
                  <input value={form.quartier} onChange={e => set("quartier", e.target.value)} placeholder="ex: Bastos, Bonanjo..." style={inp()} />
                </div>
                <div>
                  <label style={{ color: "rgba(244, 235, 220, 0.62)", fontSize: 11, fontWeight: 600, display: "block", marginBottom: 5 }}>ADRESSE COMPLÈTE</label>
                  <input value={form.address} onChange={e => set("address", e.target.value)} placeholder="Rue, BP..." style={inp()} />
                </div>
              </div>
              <PhoneInput value={form.phone} onChange={v => set("phone", v)} />
              <div>
                <label style={{ color: "rgba(244, 235, 220, 0.62)", fontSize: 11, fontWeight: 600, display: "block", marginBottom: 5 }}>EMAIL</label>
                <input type="email" value={form.email} onChange={e => set("email", e.target.value)} placeholder="contact@centre.cm" style={inp()} />
                {errors.email && <div style={{ color: "#E5B4B7", fontSize: 11, marginTop: 4 }}>{errors.email}</div>}
              </div>
              <div>
                <label style={{ color: "rgba(244, 235, 220, 0.62)", fontSize: 11, fontWeight: 600, display: "block", marginBottom: 5 }}>SITE WEB</label>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
                {[
                  ...
                  <div key={k as string}>
                    <label style={{ color: "rgba(244, 235, 220, 0.62)", fontSize: 10, fontWeight: 600, display: "block", marginBottom: 5 }}>{icon} {(k as string).toUpperCase()}</label>
          ...
          {step === 2 && (
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              <h2 style={{ margin: 0, color: "var(--creme)", fontFamily: "var(--font-fraunces), Georgia, sans-serif", fontSize: 17 }}>Capacité & langues</h2>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
                <div>
                  <label style={{ color: "rgba(244, 235, 220, 0.62)", fontSize: 11, fontWeight: 600, display: "block", marginBottom: 5 }}>NB ENSEIGNANTS</label>
                  <input type="number" min="1" value={form.nbTeachers} onChange={e => set("nbTeachers", e.target.value)} placeholder="ex: 5" style={inp()} />
                </div>
                <div>
                  <label style={{ color: "rgba(244, 235, 220, 0.62)", fontSize: 11, fontWeight: 600, display: "block", marginBottom: 5 }}>NB ÉLÈVES</label>
                  <input type="number" min="1" value={form.nbStudents} onChange={e => set("nbStudents", e.target.value)} placeholder="ex: 80" style={inp()} />
                </div>
                <div>
                  <label style={{ color: "rgba(244, 235, 220, 0.62)", fontSize: 11, fontWeight: 600, display: "block", marginBottom: 5 }}>NB SALLES</label>
              ...
              <div>
                <label style={{ color: "rgba(244, 235, 220, 0.62)", fontSize: 11, fontWeight: 600, display: "block", marginBottom: 8 }}>HORAIRES D'OUVERTURE</label>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ color: "var(--creme-mute)", fontSize: 11, marginBottom: 4 }}>Ouverture</div>
                    <input type="time" value={form.openMorning} onChange={e => set("openMorning", e.target.value)} style={{ ...inp(), colorScheme: "dark" }} />
                  </div>
                  <div style={{ color: "var(--creme-mute)", fontSize: 18, paddingTop: 20 }}>—</div>
                  <div style={{ flex: 1 }}>
                    <div style={{ color: "var(--creme-mute)", fontSize: 11, marginBottom: 4 }}>Fermeture</div>
          ...
          {step === 3 && (
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              <h2 style={{ margin: 0, color: "var(--creme)", fontFamily: "var(--font-fraunces), Georgia, sans-serif", fontSize: 17 }}>Inviter l'équipe de gestion</h2>
              <p style={{ margin: 0, color: "rgba(244, 235, 220, 0.62)", fontSize: 13 }}>Invitez jusqu'à 5 administrateurs pour gérer votre centre.</p>
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {form.admins.map((admin, i) => (
                  <div key={i} style={{ display: "flex", gap: 10, alignItems: "flex-end" }}>
                    <div style={{ flex: 1 }}>
                      {i === 0 && <label style={{ color: "rgba(244, 235, 220, 0.62)", fontSize: 11, fontWeight: 600, display: "block", marginBottom: 5 }}>EMAIL</label>}
                      <input value={admin.email} onChange={e => setAdmin(i, "email", e.target.value)} placeholder="email@centre.cm" style={inp()} />
                    </div>
                    <div style={{ width: 130 }}>
                      {i === 0 && <label style={{ color: "rgba(244, 235, 220, 0.62)", fontSize: 11, fontWeight: 600, display: "block", marginBottom: 5 }}>RÔLE</label>}
                    ...
                    {i > 0 && (
                      <button onClick={() => removeAdmin(i)} style={{ background: "none", border: "none", color: "#E5B4B7", cursor: "pointer", fontSize: 18, paddingBottom: 8 }}>×</button>
                    )}
                  </div>
                ))}
              </div>
              {form.admins.length < 5 && (
                <button onClick={addAdmin} style={{ background: "rgba(184, 135, 62, 0.06)", border: "1px dashed rgba(184, 135, 62, 0.25)", borderRadius: 10, padding: "10px", color: "var(--brass)", cursor: "pointer", fontSize: 13, fontWeight: 600 }}>
              ...
              <div style={{ background: "rgba(245,158,11,0.06)", border: "1px solid rgba(245,158,11,0.15)", borderRadius: 10, padding: "12px 14px" }}>
                ...
                <div style={{ color: "rgba(244, 235, 220, 0.62)", fontSize: 12, marginTop: 4 }}>Chaque personne invitée recevra un email pour créer son compte avec le rôle assigné.</div>
              </div>
            </div>
          )}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              {form.payStep === "done" ? (
                <div style={{ textAlign: "center", padding: "32px 0" }}>
                  ...
                  <div style={{ color: "var(--brass)", fontFamily: "var(--font-fraunces), Georgia, sans-serif", fontWeight: 800, fontSize: 22 }}>Centre créé avec succès !</div>
                  <div style={{ color: "rgba(244, 235, 220, 0.62)", fontSize: 13, marginTop: 8 }}>Redirection vers votre tableau de bord...</div>
                </div>
              ) : form.payStep === "confirm" ? (
                <>
                  <h2 style={{ margin: 0, color: "var(--creme)", fontFamily: "var(--font-fraunces), Georgia, sans-serif", fontSize: 17 }}>Confirmer le paiement</h2>
                  <div style={{ background: "rgba(184, 135, 62, 0.06)", border: "1px solid rgba(184, 135, 62, 0.2)", borderRadius: 12, padding: "16px 20px" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                      <span style={{ color: "rgba(244, 235, 220, 0.72)", fontSize: 13 }}>Plan</span>
                      <span style={{ color: "var(--creme)", fontWeight: 700 }}>Yema {selectedPlan.name}</span>
                    </div>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                      <span style={{ color: "rgba(244, 235, 220, 0.72)", fontSize: 13 }}>Montant</span>
                      <span style={{ color: "var(--brass)", fontWeight: 700, fontSize: 18 }}>{selectedPlan.price.toLocaleString()} XAF</span>
                    </div>
                    <div style={{ display: "flex", justifyContent: "space-between" }}>
                      <span style={{ color: "rgba(244, 235, 220, 0.72)", fontSize: 13 }}>Numéro facturé</span>
                      <span style={{ color: "var(--creme)", fontFamily: "monospace" }}>+237 {form.payPhone}</span>
                    </div>
                  </div>
                  <div style={{ color: "rgba(244, 235, 220, 0.62)", fontSize: 12, textAlign: "center" }}>
                    Vous recevrez une notification {form.payMethod === "mtn" ? "MTN Mobile Money" : "Orange Money"} pour confirmer.
                  </div>
                </>
              ) : (
                <>
                  <h2 style={{ margin: 0, color: "var(--creme)", fontFamily: "var(--font-fraunces), Georgia, sans-serif", fontSize: 17 }}>Choisir votre abonnement</h2>
                    ...
                    {PLANS.map(p => {
                      ...
                      return (
                        <button key={p.id} onClick={() => set("plan", p.id)} style={{
                          ...
                          <div style={{ flex: 1 }}>
                            <div style={{ display: "flex", justifyContent: "space-between" }}>
                              <span style={{ color: active ? "var(--brass)" : "var(--creme)", fontWeight: 700, fontSize: 15 }}>{p.name}</span>
                              <span style={{ color: active ? "var(--brass)" : "var(--creme)", fontWeight: 800, fontSize: 15 }}>{p.price.toLocaleString()} XAF<span style={{ color: "var(--creme-mute)", fontSize: 11, fontWeight: 400 }}>/mois</span></span>
                            </div>
                            <div style={{ color: "rgba(244, 235, 220, 0.62)", fontSize: 11, marginTop: 4 }}>
                              {p.admins} admins · {p.teachers} enseignants · {p.students} élèves
                            </div>
                          </div>
                          {active && <span style={{ color: "var(--brass)", flexShrink: 0 }}>✓</span>}
                        </button>
                      );
                    })}
                  </div>

                  <div style={{ background: "rgba(184, 135, 62, 0.04)", border: "1px solid rgba(184, 135, 62, 0.08)", borderRadius: 10, padding: "14px 16px" }}>
                    <div style={{ color: "var(--brass)", fontWeight: 700, fontSize: 13, marginBottom: 10 }}>Mode de paiement</div>
                    <div style={{ display: "flex", gap: 10 }}>
                      {[{ v: "mtn", label: "MTN Money", icon: "🟡" }, { v: "orange", label: "Orange Money", icon: "🟠" }].map(m => (
                        <button key={m.v} onClick={() => set("payMethod", m.v)} style={{
                          ...
                          background: form.payMethod === m.v ? "rgba(184, 135, 62, 0.12)" : "rgba(244, 235, 220, 0.03)",
                          border: `1px solid ${form.payMethod === m.v ? "rgba(184, 135, 62, 0.3)" : "rgba(244, 235, 220, 0.09)"}`,
                          color: form.payMethod === m.v ? "var(--brass)" : "rgba(244, 235, 220, 0.72)", fontWeight: 600, fontSize: 13,
                        }}>{m.icon} {m.label}</button>
                      ))}
                    </div>
                    {form.payMethod && (
                      <div style={{ marginTop: 12 }}>
                        <label style={{ color: "rgba(244, 235, 220, 0.62)", fontSize: 11, fontWeight: 600, display: "block", marginBottom: 5 }}>NUMÉRO {form.payMethod.toUpperCase()}</label>
                  ...
                  <button onClick={async () => {
                    ...
                    document.cookie = "onboarding_done=true;path=/;max-age=2592000";
                    router.push("/center");
                  }} style={{ background: "none", border: "none", color: "var(--creme-mute)", cursor: "pointer", fontSize: 12, textDecoration: "underline", textAlign: "center" }}>
          ...
          {form.payStep !== "done" && (
            <div style={{ display: "flex", gap: 10, justifyContent: "space-between", marginTop: 24 }}>
              {step > 0 && form.payStep === "form" ? (
                ...
                <button onClick={() => set("payStep", "form")} style={{ background: "rgba(244, 235, 220, 0.04)", color: "rgba(244, 235, 220, 0.62)", border: "1px solid rgba(244, 235, 220, 0.09)", borderRadius: 10, padding: "12px 20px", cursor: "pointer", fontSize: 13 }}>← Modifier</button>
              ) : <div />}
              <button onClick={saveStep} disabled={saving} style={{ background: "var(--brass)", color: "var(--espresso)", border: "none", borderRadius: 10, padding: "12px 28px", fontWeight: 700, fontSize: 14, cursor: "pointer" }}>
