// P4.3a · GET /api/center/enrollments · pending join requests · pagination.

import { NextResponse } from "next/server";
import type { NextRequest } from "next/server";
import { isCenterRealDataActive } from "@/lib/flags";
import { resolveCenterActor } from "@/lib/permissions/center";
import { getCenterPendingEnrollments } from "@/lib/center/queries";
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const result = await getCenterPendingEnrollments(actor.centerId, {
