...
describe("German A1 complete validation flow", () => {
  it("completes all twelve units in sequence with the expected XP and no duplicate rewards", () => {
    ...
    expect(audit.completedUnits).toBe(12);
    expect(audit.totalUnits).toBe(12);
    expect(audit.overallProgress).toBe(100);
    expect(audit.xpAwarded).toBe(audit.expectedXp);
    expect(audit.xpAwarded).toBe(1440);
